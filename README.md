...
Camera preferences include an **FPS** selector for Auto, 15, 24, 30, or 60 FPS. Changing it safely reconnects the camera and shows the frame rate reported by the driver. Manual focus and focus lock are enabled only when the camera driver exposes those controls. On Windows, **Camera controls…** opens the driver's native settings as a fallback for cameras that do not expose focus through OpenCV.
